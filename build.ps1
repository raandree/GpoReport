...
begin
{
    ...
    # Set the alias for Sampler task variable script
    $samplerModule = Get-Module -Name 'Sampler' -ListAvailable | Select-Object -First 1
    if ($samplerModule) {
        $samplerScriptPath = Join-Path -Path $samplerModule.ModuleBase -ChildPath 'scripts/Set-SamplerTaskVariable.ps1'
        if (Test-Path -Path $samplerScriptPath) {
            Set-Alias -Name 'Set-SamplerTaskVariable' -Value $samplerScriptPath
        }
    }
}
